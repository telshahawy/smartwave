...
{
    public class GetAvailableVisitsForChemistQueryHandler : IQueryHandler<IGetAvailableVisitsForChemistQuery, IGetAvailableVisitsInAreaQueryResponse>
    {
        private readonly HomeVisitsReadModelContext _context;
        private readonly ILog _log;

        public GetAvailableVisitsForChemistQueryHandler(HomeVisitsReadModelContext context, ILog log)
        {
            _context = context;
            _log = log;
        }

        public IGetAvailableVisitsInAreaQueryResponse Read(IGetAvailableVisitsForChemistQuery query)
        {
            ...
            if (query != null)
            {
                dbQuery = dbQuery.Where(x => x.ChemistId == query.ChemistId && (query.GeoZoneId == null || x.GeoZoneId == query.GeoZoneId) && !x.BranchDispatch
                && query.Date.Date >= x.ScheuleStartDate.Date && query.Date.Date < x.ScheduleEndDate.Date && x.ChemistClientId == query.ClientId && x.Day == DayOfWeek(query.Date, System.DayOfWeek.Sunday));

                if (dbQuery == null)
                    throw new Exception("Chemist not found");

                var visitsGroupQuery = visitsDbQuery.Where(x => x.VisitDate.Date == query.Date.Date && x.ClientId == query.ClientId && (query.GeoZoneId == null || x.GeoZoneId == query.GeoZoneId) &&
                x.ChemistId == query.ChemistId && /*x.VisitStatus != (int)VisitStatusTypes.Done &&*/ x.VisitStatus != (int)VisitStatusTypes.Cancelled
                && x.VisitStatus != (int)VisitStatusTypes.Reject).ToList();

                int reseved = visitsGroupQuery.Count() > 0 ? visitsGroupQuery.Count() : 0;

                onHoldVisitsViews = _context.OnHoldVisitsViews.Where(x => x.ChemistId == query.ChemistId && x.CreatedAt >= currentDate.AddMinutes(-3) && x.CreatedAt <= currentDate && !x.IsCanceled);
                //   onHoldVisitsViews = _context.OnHoldVisitsViews.Where(x =>  x.CreatedAt <= currentDate && !x.IsCanceled);
                ...
                if (dbQuery.Count() > 0)
                {
                    ...
                    if (reseved > 0)
                    {
                        ...
                        return new GetAvailableVisitsInAreaQueryResponse()
                        {
                            AvailableVisits = ZoneAvailability.Where(x => x.IsAvialableTimeZone).Select(x => new AvailableVisitsInAreaDto
                            {
                                ...
                                AvalailableVisits = (x.TimeZoneQouta - visitsGroupQuery.Where(g => g.TimeZoneFrameId == x.TimeZoneFrameGeoZoneId && g.ChemistId == query.ChemistId).Count()) > 0 ? (x.TimeZoneQouta - visitsGroupQuery.Where(g => g.TimeZoneGeoZoneId == x.TimeZoneFrameGeoZoneId && g.ChemistId == query.ChemistId).Count()) : 0,
                    ...
                    else
                    {
                        ...
                        return new GetAvailableVisitsInAreaQueryResponse()
                        {
                            ...
                            {
                                GeoZoneName = x.GeoZoneName,
                                //AvalailableVisits = (x.TotalChemistTime - x.TotalReservedVisits) >0 ? (x.TotalChemistTime - x.TotalReservedVisits):0,
                                //AvalailableVisits = (x.TimeZoneQouta - visitsGroupQuery.Where(g => g.TimeZoneGeoZoneId == x.TimeZoneFrameGeoZoneId && g.ChemistId == query.ChemistId).Count()) > 0 ? (x.TimeZoneQouta - visitsGroupQuery.Where(g => g.TimeZoneGeoZoneId == x.TimeZoneFrameGeoZoneId && g.ChemistId == query.ChemistId).Count()) : 0,
        ...
        private bool IsAvialableTimeZone(ChemistTimeZoneAvailabilityView chemistTimeZoneAvailabilityView, DateTime visitDate, List<GetAllVisitsView> allVisitsView, SystemParametersView systemParameter)
        {
            ...
            //Validate time zone with chemist visits
