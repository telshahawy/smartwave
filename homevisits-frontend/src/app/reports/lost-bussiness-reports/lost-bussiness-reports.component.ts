...
@Component({
  selector: 'app-lost-bussiness-reports',
  templateUrl: './lost-bussiness-reports.component.html',
  styleUrls: ['./lost-bussiness-reports.component.css'],
})
export class LostBussinessReportsComponent extends FormManage implements OnInit {
  constructor(
    ...
    super(PagesEnum.LostBusinessReport, ActionsEnum.View, router, notify);
  }
  ...
  onSubmit() {
    ...
    if (visit.areaId) {
      ...
      this.createLostBusinessReport(searchData);
  }

  createLostBusinessReport(visit) {
    this.service.reportsService.getVisitReject(visit).subscribe(res => {
      this.responseSuccess(res)
      this.router.navigate(['/client/reports/lost-bussiness-reports/total']);
