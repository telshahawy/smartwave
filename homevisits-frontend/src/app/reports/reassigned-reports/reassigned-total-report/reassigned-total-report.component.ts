...
@Component({
  selector: 'app-reassigned-total-report',
  templateUrl: './reassigned-total-report.component.html',
  styleUrls: ['./reassigned-total-report.component.css']
})
export class ReassignedTotalReportComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    super(PagesEnum.RejectedReport,ActionsEnum.View,router,notify)
  }
  ngOnInit(): void {
  ...
  search(page?: PageEvent) {
    ...
    this.service.reportsService.visitSubject$.subscribe(res=>{
      if(res){
        this.visitList=res
      if (res?.rejectedVisitReports?.length > 0) {
        
        this.loading = false;
        this.data = res;
        this.dataSource = new MatTableDataSource<any>(
          this.data?.rejectedVisitReports
        );
        this.dataSource.paginator = this.paginator;
        this.dataSource.sort = this.sort;
        this.isShowEmpty = false;
        ...
        this.router.navigate(['/client/reports/reassigned-reports/create']);
