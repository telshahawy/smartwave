...
import { AreaLookup, CountryLookup, CreatePatientAddress, Gender, GovernateLookup, Phones, SearchPatientsList, SearchPatientsSendToParent } from 'src/app/core/models/models';
...
@Component({
  selector: 'app-user-patient-create',
  templateUrl: './user-patient-create.component.html',
  styleUrls: ['./user-patient-create.component.css']
})
export class UserPatientCreateComponent implements OnInit {
  genderType = Gender;
  gender:number=null;
  phones:Phones[]=[];
   ...
   this.phone=new Phones();
    ...
    if (dto) {
      ...
      var date2  = formatDate(dto.birthDate,'yyyy-MM-dd','en_US');
       if(date2 > date1)
       {
        this.notify.error("You Must Choose Date of Birth Less Than Today", 'FAILED OPERATION');
      ...
      this.service.createPatient(dto).subscribe(
        (res) => {
          ...
          if(res.responseCode==1)
          {
           ...
           this.router.navigate(['users/patients-data', patientId]);
